...
type
  ...
  public
    constructor Create;
    destructor Destroy; override;
    procedure EnsureRemoteRepository(const ASourcePath: string; const ARemoteUrl: string);
    procedure EnsureConfigUserData(const ASourcePath: string);
    procedure EnsureUserAuthentication(const ASourcePath: string; const ARemoteUrl: string);
    ...
    function CheckRemoteStatus(const ASourcePath: string; ARemoteUrl: string; out RemoteState: string): Boolean;
    function ResetRemoteUrl(const ASourcePath, ARemoteUrl: string): Boolean;
    function ResetRemoteBranchMaster(const ASourcePath, ARemoteUrl: string): Boolean;
...
uses Controls, System.Win.Registry, VerInsCryptUtils, ToolsApi, Dialogs,
  GitIDEConst, GitClientLoginPrompt, TLHelp32;
...
function InternalExecute(CommandLine: string; var Output: string; OutputLineCallback: TTextHandler;
  RawOutput: Boolean; AbortPtr: PBoolean; const CurrentDir: string; const TimeOut: Cardinal = 5000; const RemoteAccess: Boolean = False): Cardinal;

const
  BufferSize = 255;
  sTimeOutExpired = 'Timeout expired. ' + #13;
type
  ...
  procedure KillChildProcesses(const ParentProcId: Cardinal);
  var
    hSnapshot: THandle;
    hChildProc: THandle;
    ProcEntry: TProcessEntry32;
    ChildProcId: Cardinal;
  begin
    ProcEntry.dwSize := Sizeof(TProcessEntry32);
    hSnapshot := CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);

    if (hSnapshot <> INVALID_HANDLE_VALUE)
      and Process32First(hSnapshot, ProcEntry) then
    begin
      ChildProcId := $FFFFFFFF;
      while Process32Next(hSnapshot, ProcEntry) do
      begin
        if (ProcEntry.th32ParentProcessID = ParentProcId)
          or (ProcEntry.th32ParentProcessID = ChildProcId) then
        begin
          ChildProcId := ProcEntry.th32ProcessID;
          hChildProc := OpenProcess(PROCESS_ALL_ACCESS, FALSE, ProcEntry.th32ProcessID);
    ...
    begin
      ...
      begin
        ...
        repeat
          ...
          PeekNamedPipe(PipeRead, @Buffer, BufferSize, @bytesRead, @bytesAvail, nil);
          if bytesRead > 0 then
          begin
            // Reset TimeOut
            AbortTimeOut := 0;
            ReadFile(PipeRead, Buffer, BufferSize, PipeBytesRead, nil);
            if (PipeBytesRead > 0) then
              ProcessBuffer(Buffer, Line, PipeBytesRead);
          end
          else
            // No data was read, so start a time out
            if AbortTimeOut = 0 then
              AbortTimeOut := GetTickCount + TimeOut;
        until (Running <> WAIT_TIMEOUT) or ((AbortTimeOut <> 0) and (GetTickCount > AbortTimeOut))
          or ((AbortPtr <> nil) and LongBool(AbortPtr^));

        TimeOutExpired := ((AbortTimeOut <> 0) and (GetTickCount > AbortTimeOut));

        if ((AbortPtr <> nil) and LongBool(AbortPtr^)) or TimeOutExpired then
        begin
          KillChildProcesses(ProcessInfo.dwProcessId);
          if ((AbortPtr <> nil) and LongBool(AbortPtr^)) then
            TerminateProcess(ProcessInfo.hProcess, Cardinal(ABORT_EXIT_CODE));
          if TimeOutExpired then
          begin
            TerminateProcess(ProcessInfo.hProcess, Cardinal(WAIT_TIMEOUT));
            ProcessLine(sTimeOutExpired, Length(sTimeOutExpired));
          end;
        end;
      end;
...
function Execute(const CommandLine: string; var Output: string; RawOutput: Boolean = False;
  AbortPtr: PBoolean = nil; const CurrentDir: string = ''; const TimeOut: Cardinal = 5000; const RemoteAccess: Boolean = False): Cardinal; overload;
begin
  Result := InternalExecute(CommandLine, Output, nil, RawOutput, AbortPtr, CurrentDir, TimeOut, RemoteAccess);
end;

function Execute(const CommandLine: string; OutputLineCallback: TTextHandler; RawOutput: Boolean = False;
  AbortPtr: PBoolean = nil; const CurrentDir: string = ''; TimeOut: Cardinal = 5000; const RemoteAccess: Boolean = False): Cardinal; overload;
var
  Dummy: string;
begin
  Dummy := '';
  Result := InternalExecute(CommandLine, Dummy, OutputLineCallback, RawOutput, AbortPtr, CurrentDir, TimeOut, RemoteAccess);
end;
...
begin
  ...
  if (RemoteUserName <> '') and (RemoteUserPass <> '') then
    FormattedUrl := FormatUrl(ASourcePath, RemoteUserName, RemoteUserPass)
  else
    FormattedUrl := ASourcePath;

  CmdLine := GitExecutable + ' clone -v --progress ';
  CmdLine := CmdLine + QuoteFileName(FormattedUrl) + ' .';
  try
    FCancel := False;
    Res := Execute(CmdLine, ExecuteTextHandler, False, @FCancel, CurrentDir, 10000, True);
    // Set remote.origin.url to the original URL to avoid showing the formatted url
    // with the user credentials through git config --list from command line.
    if Res = 0 then
      ResetRemoteUrl(ADestPath, ASourcePath);
   finally
    FCloneCallBack := nil;
  end;
  Result := Res = 0;
...
begin
  // TODO: JuanR. Investigate the subprocess I/O redirection that uses pipes
  // ReadFile winapi function hangs when cloning a private repository and the password is prompted.
  // By the moment the workaround is to format the url by including the user credentials.
  {
    - Sample base url formats:
      https://github.com/EmbTech/Test.git
      https://bitbucket.org/JuanREmb/test
      https://juanremb@bitbucket.org/juanremb/test.git

    - The final url format should be:
      https://UserName:Password@github.com/EmbTech/Test.git
      https://UserName:Password@bitbucket.org/juanremb/test.git
...
procedure TGitClient.EnsureRemoteRepository(const ASourcePath: string; const ARemoteUrl: string);
var
  CmdLine, Output: string;
  Res: Integer;
begin
  ...
  begin
    // The local repository does not exits.
    Output := ARemoteUrl;
    Output := Output.Replace(sLineBreak, '');
    RemoteUrl := Output;
    RemoteName := Copy(Output, Output.LastIndexOf('/') + 2, MaxInt);
...
procedure TGitClient.EnsureUserAuthentication(const ASourcePath: string; const ARemoteUrl: string);
var
  UName, UPass: string;
  Save: Boolean;
begin
  EnsureRemoteRepository(ASourcePath, ARemoteUrl);
  // Reset user credentials in memory and load it from registry
  RemoteUserName := '';
  RemoteUserPass := '';
  LoadRemoteInfo;
...
var
  ...
  SubKey: string;
begin
  if (RemoteUrl <> '') And (RemoteUserName <> '') And (RemoteUserPass <> '') then
  begin
...
begin
  ...
  if RegIniFile.OpenKey(Key, False) then
  try
    //RemoteUrl := RegIniFile.ReadString('', sRemoteUrl, '');
...
var
  Res: Integer;
  CmdLine: string;
  FormattedUrl, RemoteStatus: string;
begin
  Result := False;
  EnsureRemoteRepository(ASourcePath, RemoteUrl);

  if CheckRemoteStatus(ASourcePath, RemoteUrl, RemoteStatus) then
  begin
    CmdLine := FGitExecutable + ' pull ' + RemoteUrl;
    Res := Execute(CmdLine, Output, False, nil, ASourcePath, 5000, True);
    Result := Res = 0;
  end
  else
  begin
    ...
    begin
      EnsureUserAuthentication(ASourcePath, RemoteUrl);
      if (RemoteUserName <> '') and (RemoteUserPass <> '') then
      begin
        FormattedUrl := FormatUrl(RemoteUrl, RemoteUserName, RemoteUserPass);
        if CheckRemoteStatus(ASourcePath, FormattedUrl, RemoteStatus) then
        begin
          CmdLine := FGitExecutable + ' pull ' + FormattedUrl;
          Res := Execute(CmdLine, Output, False, nil, ASourcePath, 5000, True);
          Output := Output.Replace(FormattedUrl, RemoteUrl);
          Result := Res = 0;
        end
        else
          Output := RemoteStatus.Replace(FormattedUrl, RemoteUrl);
      end
      else
...
begin
  Result := False;
  // Always required for push operations, even for public repositories
  EnsureUserAuthentication(ASourcePath, RemoteUrl);
  if (RemoteUserName = '') or (RemoteUserPass = '') then
    Output := sCanceledByUser;
  if  (RemoteUserName <> '') and (RemoteUserPass <> '') then
  begin
    ...
    if CheckRemoteStatus(ASourcePath, FormattedUrl, RemoteStatus) then
    begin
      CmdLine := FGitExecutable + ' push --all -u ' + FormattedUrl;
      Res := Execute(CmdLine, Output, False, nil, ASourcePath, 5000, True);
      Output := Output.Replace(FormattedUrl, RemoteUrl);
      Result := Res = 0;
      // Reset branch.master.remote from git config to don't show the url with user credentials
      // when user executes git config --list from command line
      ResetRemoteBranchMaster(ASourcePath, FormattedUrl);
    end
    else
      Output := RemoteStatus.Replace(FormattedUrl, RemoteUrl);
  end;
...
function TGitClient.CheckRemoteStatus(const ASourcePath: string; ARemoteUrl: string; out RemoteState: string): Boolean;
var
  CmdLine, Output: string;
  OutputStrings: TStringList;
begin
  ...
  Execute(CmdLine, Output, False, nil, ASourcePath, 5000, True);
  if Output <> '' then
  begin
    OutputStrings := TStringList.Create;
    try
      OutputStrings.Text := Output;
      RemoteState := OutputStrings[0];
      Result := (RemoteState <> '')
        and not RemoteState.Contains(sRepositoryNotFound)
        and not RemoteState.Contains(sInvalidUserNameOrPassword)
        and not RemoteState.Contains(sUnableToAccess)
        and not RemoteState.Contains(sAuthenticationFailed);
    finally
      OutputStrings.Free
    end;
  end;
...
function TGitClient.ResetRemoteBranchMaster(const ASourcePath,
  ARemoteUrl: string): Boolean;
var
  Res: Integer;
  CmdLine, Output: string;
begin
  CmdLine := FGitExecutable + ' config --get branch.master.remote ';
  Res := Execute(CmdLine, Output, False, nil, ASourcePath);
  Output := Output.Replace(sLineBreak, '');
  if SameText(Output, ARemoteUrl)  then
  begin
    CmdLine := FGitExecutable + ' config branch.master.remote ' + RemoteUrl;
    Res := Execute(CmdLine, Output, False, nil, ASourcePath);
  end;
  Result := Res = 0;
end;

function TGitClient.ResetRemoteUrl(const ASourcePath, ARemoteUrl: string): Boolean;
var
  Res: Integer;
  CmdLine, Output: string;
begin
  CmdLine := FGitExecutable + ' remote set-url origin ' + ARemoteUrl;
  Res := Execute(CmdLine, Output, False, nil, ASourcePath);
  Result := Res = 0;
end;
