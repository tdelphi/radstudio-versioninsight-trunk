...
uses Classes, GitIDEMenus, GitIDEClient, GitClient;

type
  TBaseCommitGitMenu = class(TGitMenu)
  protected
    FRootType: TRootType;
  ...
  TRootDirCommitGitMenu = class(TBaseCommitGitMenu)
  public
    constructor Create(ASvnIDEClient: TGitIDEClient);
  end;

  TProjectDirCommitGitMenu = class(TBaseCommitGitMenu)
  public
    constructor Create(ASvnIDEClient: TGitIDEClient);
  end;

  TExpicitFilesCommitGitMenu = class(TBaseCommitGitMenu)
  public
    constructor Create(ASvnIDEClient: TGitIDEClient);
  end;
  ...
  procedure DoCommit(const GitClient: TGitClient; const CommitList: TStringList;
...
type
  ...
  protected
    ...
    var
      ...
      FSvnCommitFrame: TGitCommitFrame;
      FRootType: TRootType;
    ...
    { CallBacks }
    procedure ModificationCallBack(Sender: TObject; Item: TGitItem;
      var Cancel: Boolean);
    ...
    function RevertCallBack(const FileName: string; ARecursive: Boolean; var ANewTextStatus: TGitStatus): Boolean;
    ...
    procedure StatusCallBack(Sender: TObject; Item: TGitItem; var Cancel: Boolean);
    procedure RefreshCallBack;
    procedure ModificationRefreshCallBack(Sender: TObject; Item: TGitItem;
      var Cancel: Boolean);
    { Misc }
    procedure PrepareFileList(FrameAdd: TRefreshProc; AModificationCallBack: TGitStatusCallback; var AURL: string);
  public
    constructor Create(SvnClient: TGitClient; const DirectoryList: TStringList;
      RootType: TRootType);
...
constructor TBaseCommitGitMenu.Create(ASvnIDEClient: TGitIDEClient);
begin
  inherited;
  FCaption := sPMMCommit;
  FVerb := sPMVCommit;
...
procedure TBaseCommitGitMenu.Execute(const MenuContextList: IInterfaceList);
var
  DirectoryList: TStringList;
  ProjectFound: Boolean;
begin
  if FSvnIDEClient.GitClient.GitExecutable <> '' then
  begin
    ...
    begin
      ...
      try
        BuildFileList(MenuContextList, DirectoryList, FSvnIDEClient.GitClient, FRootType, ProjectFound);
        CommitView := TCommit.Create(FSvnIDEClient.GitClient, DirectoryList, FRootType);
        (BorlandIDEServices as IOTAEditorViewServices).ShowEditorView(CommitView);
      finally
        DirectoryList.Free;
      end;
...
{ TParentCommitGitMenu }

constructor TParentCommitGitMenu.Create;
begin
...
{ TProjectDirCommitGitMenu }

constructor TProjectDirCommitGitMenu.Create(ASvnIDEClient: TGitIDEClient);
begin
  inherited Create(ASvnIDEClient);
...
{ TExpicitFilesCommitGitMenu }

constructor TExpicitFilesCommitGitMenu.Create(ASvnIDEClient: TGitIDEClient);
begin
  inherited Create(ASvnIDEClient);
...
constructor TFileCommitGitMenu.Create(ASvnIDEClient: TGitIDEClient);
begin
  inherited Create(ASvnIDEClient);
  FRootType := rtRootDir;
  FParent := sPMVGitParent;
...
begin
  ...
  try
    RevStr := Copy(SvnItem.BaseHash, 1, 7);
  ...
  (BorlandIDEServices as IOTACustomDifferenceManager).
    ShowDifference(Stream1, Stream2, FileName + '-' + RevStr,
...
procedure TCommit.PrepareFileList(FrameAdd: TRefreshProc; AModificationCallBack: TGitStatusCallback; var AURL: string);
{
...
procedure TCommit.ModificationCallBack(Sender: TObject; Item: TGitItem;
  var Cancel: Boolean);
begin
...
procedure TCommit.ModificationRefreshCallBack(Sender: TObject; Item: TGitItem;
  var Cancel: Boolean);
begin
  if (FRootType <> rtExpicitFiles) or (not FSvnCommitFrame.Found(Item.FileName)) then
  begin
...
begin
  //no idea if Git has conflicts
end;

function TCommit.RevertCallBack(const FileName: string; ARecursive: Boolean; var ANewTextStatus: TGitStatus): Boolean;
var
  Module: IOTAModule;
  SvnItem: TGitItem;
begin
...
procedure TCommit.StatusCallBack(Sender: TObject; Item: TGitItem;
  var Cancel: Boolean);
begin
...
begin
  ...
  Error := GitClient.Commit(CommitList, TrimRight(Comment), GitClient.UserName);
  if Error = geSuccess then
  begin
    SvnMessageView.WriteTitle(Format(sCommitCompleted, [GitClient.LastCommitInfoBranch + ' ' + GitClient.LastCommitInfoHash]));
    if ADeleteLocalHistory then
      DoDeleteCommitListLocalHistory(CommitList);
  end
  else
  if Error = geUnknownUserName then
    MessageDlg(sUnknownUserName, mtError, [mbOk, mbHelp], hcGitUnknownUserIdentity)
  else
  if Error = geUnknownUserEmail then
    MessageDlg(sUnknownUserEmail, mtError, [mbOk, mbHelp], hcGitUnknownUserIdentity)
  else
    MessageDlg(sUnknownError, mtError, [mbOK], 0);
