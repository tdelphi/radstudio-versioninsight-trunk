...
begin
  if Assigned(FGitLogFrame) then
    FGitLogFrame.NextCompleted;
end;
...
begin
  ...
  begin
    ...
    if SvnEditAction <> seaUnknown then
      Result := FGitLogFrame.PerformEditAction(SvnEditAction);
  end;
end;
...
begin
  FGitLogFrame := TGitLogFrame(AFrame);

  FGitItem := TGitItem.Create(FGitClient, FRootPath);
  LoadRevisionsCallBack('', '', 100);

  FGitLogFrame.FileColorCallBack := FileColorCallBack;
  FGitLogFrame.LoadRevisionsCallBack := LoadRevisionsCallBack;
  //FSvnLogFrame.ReverseMergeCallBack := ReverseMergeCallBack;
  FGitLogFrame.CompareRevisionCallBack := CompareRevisionCallBack;
  FGitLogFrame.SaveRevisionCallBack := SaveRevisionCallBack;
  FGitLogFrame.RootPath := ExcludeTrailingPathDelimiter(FRootPath);
  ...
  begin
    ...
    RootRelativePath := '';
  FGitLogFrame.RootRelativePath := RootRelativePath;
  {
...
begin
  Result := [];
  if Assigned(FGitLogFrame) then
    Result := SvnEditStateToEditState(FGitLogFrame.SvnEditState);
end;
