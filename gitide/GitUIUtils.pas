...
uses
  Windows, Messages, Controls, GitUITypes, Generics.Defaults;
...
function DateTimeToLocalDateTime(ADateTime: TDateTime): TDateTime;
function LocalDateTimeString(ADateTime: TDateTime; LongFormat: Boolean = False): string;
...
function LocalDateTimeString(ADateTime: TDateTime; LongFormat: Boolean = False): string;
begin
  ADateTime := DateTimeToLocalDateTime(ADateTime);
  if LongFormat then
    Result := FormatDateTime(FormatSettings.LongDateFormat + ' ' + FormatSettings.LongTimeFormat, ADateTime)
  else
