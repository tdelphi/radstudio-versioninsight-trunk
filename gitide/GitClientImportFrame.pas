...
{                                                                              }
{******************************************************************************}
unit GitClientImportFrame;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, StdCtrls, ExtCtrls, ComCtrls, CheckLst, GitUITypes, GitUIUtils, Clipbrd;
...
uses GitClientRecentComments;

{$R *.dfm}

procedure TGitImportFrame.BrowseClick(Sender: TObject);
var
  SURL: string;
begin
  SURL := URL.Text;
...
procedure TGitImportFrame.CMRelease(var Message: TMessage);
begin
  CloseCallBack;
end;

procedure TGitImportFrame.CommentChange(Sender: TObject);
begin
  UpdateImportButton;
end;

constructor TGitImportFrame.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TGitImportFrame.Destroy;
begin
...
procedure TGitImportFrame.GetFiles(var Files: TStringList);
var
  I: Integer;
begin
...
function TGitImportFrame.GetSvnEditState: TSvnEditState;
var
...
procedure TGitImportFrame.ImportClick(Sender: TObject);
var
  Idx: Integer;
begin
...
function TGitImportFrame.PerformEditAction(AEditAction: TSvnEditAction): Boolean;
var
  I: Integer;
  SL: TStringList;
  CustomListBox: TCustomListBox;
...
procedure TGitImportFrame.RecentMessagesClick(Sender: TObject);
var
  S: string;
begin
...
procedure TGitImportFrame.SetFiles(Files: TStringList);
begin
  CommitFiles.Items.Assign(Files);
  CommitFiles.CheckAll(cbChecked);
end;

procedure TGitImportFrame.SetRecentComments(const Value: TStringList);
begin
  FRecentComments.Assign(Value);
  RecentMessages.Enabled := FRecentComments.Count <> 0;
end;

procedure TGitImportFrame.SetURLHistory(const Value: TStringList);
begin
  FURLHistory.Assign(Value);
  URL.Items.Assign(Value);
end;

procedure TGitImportFrame.SetVersionedRoot(const Value: Boolean);
begin
...
procedure TGitImportFrame.UpdateImportButton;
begin
  Import.Enabled := ((URL.Text <> '') And (Comment.Text <> ''));
end;

procedure TGitImportFrame.URLKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  UpdateImportButton;
end;

procedure TGitImportFrame.URLSelect(Sender: TObject);
begin
  UpdateImportButton;
end;

initialization
  TGitImportFrame.FUseCount := 0;
end.
