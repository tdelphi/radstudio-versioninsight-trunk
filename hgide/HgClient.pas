...
type
  ...
  THgError = (hgeSuccess, hgeEmptyCommitMessage, hgeNoUsernameSupplied, hgeNoEmailSupplied, hgeUnknown);
...
var
  Res, I: Integer;
  CmdLine, Output, S: string;
  CurrentDir, ParentRev: string;
  FileContent: AnsiString;
  OutputStrings: TStringList;
begin
  CurrentDir := ExtractFilePath(AFileName);
  CmdLine := HgExecutable + ' cat -r ' + IntToStr(ARevision) + ' ' + QuoteFileName(ExtractFileName(AFileName));
  Res := Execute(CmdLine, Output, False, nil, CurrentDir);
  if Res <> 0 then
  begin
    // If the files was not modified in the previous revision, we need to look for the parent
    // revision to get the previous file source
    CmdLine := HgExecutable + ' log ' + QuoteFileName(ExtractFileName(AFileName));
    ...
    try
      ...
      begin
        S := OutputStrings[I];
        if S.Contains('parent:') then
        begin
          ParentRev := Copy(S, S.LastIndexOf(':') + 2, S.Length);
          Output := '';
          CmdLine := HgExecutable + ' cat -r ' + ParentRev + ' ' + QuoteFileName(ExtractFileName(AFileName));
          Res := Execute(CmdLine, Output, False, nil, CurrentDir);
          if Res = 0 then
            Break;
        end;
