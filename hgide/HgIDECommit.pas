...
begin
  if FSvnIDEClient.HgClient.HgExecutable <> '' then
  begin
    ...
    MessageDlg(sHgClientNotConfigured, mtError, [mbOk,mbHelp], hcHgClone);
end;
...
begin
  ...
  if Error = hgeNoUsernameSupplied then
    MessageDlg(sUnknownUserName, mtError, [mbOK, mbHelp], hcHgUnknownUserIdentity)
  else
  if Error = hgeNoEmailSupplied then
    MessageDlg(sUnknownUserEmail, mtError, [mbOK, mbHelp], hcHgUnknownUserIdentity)
  else
    MessageDlg('unknown error', mtError, [mbOK, mbHelp], 0);
