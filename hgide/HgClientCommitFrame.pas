...
{                                                                              }
{******************************************************************************}
unit HgClientCommitFrame;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, ComCtrls, StdCtrls, HgClient, Generics.Collections, ActnList, Menus,
  ExtCtrls, HgUITypes, HgUIUtils, Clipbrd, Generics.Defaults, HgImages, HgTree;
...
type
  ...
  protected
    ...
    FDirectory: Boolean;
    FPathName: string;
    FTextStatus: THgStatus;
    FChecked: Boolean;
    FVisible: Boolean;
    procedure SetTextStatus(Value: THgStatus);
  public
    constructor Create(const APathName: string; ATextStatus: THgStatus;
      ADirectory, ACopied: Boolean);
    procedure NewValues(const APathName: string; ATextStatus: THgStatus;
      ADirectory, ACopied: Boolean);
    property Copied: Boolean read FCopied;
    property Directory: Boolean read FDirectory;
    property PathName: string read FPathName;
    property TextStatus: THgStatus read FTextStatus write SetTextStatus;
  ...
  TRevertCallBack = function(const FileName: string; ARecursive: Boolean; var ANewTextStatus: THgStatus): Boolean of object;
...
uses {HgClient, }HgUIConst, HgClientRecentComments;
...
procedure THgCommitFrame.EnableAlternativeLayout;
var
      ...
      for I := 0 to MissingList.Count - 1 do
        ...
        begin
          ...
          try
            ...
            if SvnListViewItem.FTextStatus <> gsMissing then
            begin
...
constructor TSvnListViewItem.Create(const APathName: string;
  ATextStatus: THgStatus; ADirectory, ACopied: Boolean);
begin
  inherited Create;
...
procedure TSvnListViewItem.SetTextStatus(Value: THgStatus);
begin
  FTextStatus := Value;
  FChecked := (FTextStatus <> gsUnversioned) and
    {(FTextStatus <> svnWcStatusExternal) and }(FTextStatus <> gsMissing) {and
