...
{******************************************************************************}
{                                                                              }
{ This unit contains import declarations for libapr.dll, Apache Portable       }
{ Runtime 0.9.12 DLL.                                                          }
{                                                                              }
{******************************************************************************}
unit apr;
...
function LoadDll(const BaseFileName, FileName, Path: string): THandle;

function AprLibLoaded: Boolean;
function LoadAprLib(const FileName: string = ''; const PathName: string = ''): Boolean;
...
begin
  ...
  if LibFileName = '' then
    ...
    Result := LoadLibraryEx(PChar(LibFileName), 0, LOAD_WITH_ALTERED_SEARCH_PATH);
end;
...
begin
  ...
  if Result then
  begin
    AprLib := LoadDll(sLibAprDll, FileName, PathName);
    Result := AprLib <> 0;
    if not Result then
